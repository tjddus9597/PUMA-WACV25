...
from models.prompt import PromptPool, VPT, StochasticAdapter, AdaptFormer, Lora
...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False, prefix=None, lora=None, l=0):
        B, N, C = x.shape
        if lora is not None:
            lq, lk = lora(x, l, 0), lora(x, l, 1)

        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        if prefix is not None:
            Ek = prefix[:,:int(prefix.size(1)/2),:]
            Ev = prefix[:,int(prefix.size(1)/2):,:]
        ...
        if lora is not None:
            lq = lq.reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            lk = lk.reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            q, k = q + lq, k + lk
...
class Block(nn.Module):
    ...
    def forward_attn(self, x, prefix=None, lora=None, l=0):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), prefix=prefix, lora=lora, l=l)))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        use_prompt, prompt_type, prefix_style, prompt_deep, component_size, prompt_length = prompt_config
        use_adapter, adapter_type, adapter_dim, adapter_droppath = adapter_config
        use_lora, lora_dim, lora_droppath = lora_config

        self.prefix_style = prefix_style
        self.prompt_deep = prompt_deep
        self.component_size = component_size if component_size is not None else 0
        self.prompt_length = prompt_length if prompt_length is not None else 0
        prompt_param = [component_size, prompt_length]

        if use_prompt:
            if prompt_type == 'pool':
                self.prompt = PromptPool(prompt_param, embed_dim, embed_dim, prompt_deep)
            elif prompt_type == 'vpt':
                self.prompt = VPT(prompt_param, embed_dim, embed_dim, prompt_deep)
            else:
                raise NotImplementedError
        else:
            self.prompt = None
        embed_len += self.prompt_length
        ...
        if use_adapter:
            if adapter_type == 'stochastic':
                self.adapter = StochasticAdapter(embed_dim, low_dim = adapter_dim, drop_path=adapter_droppath)
            elif adapter_type == 'mixture':
                self.adapter = MoEAdapter(embed_dim, low_dim = adapter_dim, drop_path=adapter_droppath)
            elif adapter_type == 'adaptformer':
                self.adapter = AdaptFormer(embed_dim, low_dim = adapter_dim)
            else:
                raise NotImplementedError
        else:
            self.adapter = None
        ...
        if use_lora:
            self.lora = Lora(embed_dim, low_dim = lora_dim, drop_path=lora_droppath)
        else:
            self.lora = None
    ...
    def forward_head(self, x, pre_logits: bool = False):
        if self.pooling:
            num_prefix_tokens = self.num_prefix_tokens if self.prefix_style or self.prompt is None else self.num_prefix_tokens + self.prompt_length 
            if self.pooling == 'avg':
                x = x[:, self.num_prefix_tokens:self.prompt_length].mean(dim=1)
            elif self.pooling == 'max':
                x = x[:, self.num_prefix_tokens:self.prompt_length].max(dim=1)[0]
            elif self.pooling == 'avgmax':
                x = x[:, self.num_prefix_tokens:self.prompt_length].mean(dim=1) + x[:, self.num_prefix_tokens:self.prompt_length].max(dim=1)[0]
            else: 
                x = x[:, 0]
        x = self.fc_norm(x)
    ...
    def forward(self, x, q=None):
        x, x_query, loss = self.forward_features(x, q)
...
def resize_pos_embed(posemb, posemb_new, num_prefix_tokens=1, gs_new=()):
    ...
    if num_prefix_tokens:
        posemb_prefix, posemb_grid = posemb[:, :1], posemb[0, num_prefix_tokens:]
